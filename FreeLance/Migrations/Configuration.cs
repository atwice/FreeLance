    ...
    internal sealed class Configuration : DbMigrationsConfiguration<FreeLance.Models.ApplicationDbContext>
    {
        ...
        private ProblemModels addProblem(ApplicationDbContext context, string name, string desc, ProblemStatus status, ApplicationUser employer)
        {
            ...
            var problem = new ProblemModels
            {
                ...
                Description = desc + additionalDescription,
                Status = status,
                Employer = employer,
        ...
        private void SeedUsers(ApplicationDbContext context)
        {
            ...
            addUser(context, userManager, "Alice Wonderful", "alicefreelancer@ya.ru", "111111", "Freelancer");
        }
        ...
        private void SeedProblemSupertask(FreeLance.Models.ApplicationDbContext context)
        {
           
                var userManager = new UserManager<ApplicationUser>(new UserStore<ApplicationUser>(context));
                var superEmployer = addUser(context, userManager, "Mr SuperEmployer", "superemployer@ya.ru", "111111",
                    "Employer");
                var problemSupertask = addProblem(context, "Super Hard Task", "Write Operation System.", ProblemStatus.Opened,
                    superEmployer);
                var aliceFreelancer = userManager.FindByEmail("alicefreelancer@ya.ru");
                var documents = new DocumentPackageModels { 
                    Freelancer = aliceFreelancer, Adress = "St. Mira, house 5, flat 11", Phone="+7624420240420", PaymentDetails = "Nothing is payed"};
                var contractsSupertask = new List<ContractModels>();
                var contractSupertask = addContract(context,
                    "It is strong and impossible task, but I believe in you, like one day my employer believed in me. You can do this! Good luck..",
                    ContractStatus.ClosedNotPaid, problemSupertask, aliceFreelancer);
                contractsSupertask.Add(contractSupertask);
                problemSupertask.Contracts = contractsSupertask;
                context.ProblemModels.AddOrUpdate(p => p.Name, problemSupertask);
                context.DocumentPackageModels.Add(documents);
                context.SaveChanges();
